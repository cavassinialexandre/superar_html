...
interface V1EmptyStateProps {
  reason: HistoryDataResult['emptyReason']
  onClear: () => void
}

export function V1EmptyState({ reason, onClear }: V1EmptyStateProps) {
  const config = (() => {
    switch (reason) {
      case 'no-data':
        return {
          ...
          description:
            'Quando auditorias e follow-ups forem aplicados a grupos desta unidade, eles aparecerão aqui na linha do tempo.',
          actionLabel: null,
        }
        ...
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 200" preserveAspectRatio="xMidYMid slice">
          <defs>
            <linearGradient id="emptyGrad" x1="0" y1="0" x2="1" y2="1">
              ...
              <stop offset="100%" stopColor="#96D4D0" />
            </linearGradient>
          </defs>
          <path d="M0 100 Q100 60 200 100 T400 100" stroke="url(#emptyGrad)" strokeWidth="2" fill="none" />
        </svg>
      </div>

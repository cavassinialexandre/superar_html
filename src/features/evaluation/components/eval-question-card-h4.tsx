/**
 * Variant H4 -- "Floating Action Bar"
 *
 * Identical to H (Elevated Float Card) for question text, number badge,
 * metadata, answer selector, and justification. Replaces the compact
 * AttachmentZone with a centered floating action bar at the bottom of
 * the card. Two circular icon buttons (comment, attachment) that toggle
 * popover-like content panels above the bar with spring animations.
...
function CommentBubbleIcon({ size = 14 }: { size?: number }) {
  return (
...
type ActivePopover = 'comment' | 'attachment' | null

// ---------------------------------------------------------------------------
// Component
// ---------------------------------------------------------------------------

export function EvalQuestionCardH4({
  question,
  index,
  state,
  onAnswer,
  ...
  // Floating action bar popover state
  ...
  function handleTogglePopover(popover: ActivePopover) {
    setActivePopover((prev) => (prev === popover ? null : popover))
  }

  const commentCount = state?.comment?.trim() ? 1 : 0
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ---- H4 Floating Action Bar ---- */}

        {/* Popover content area (expands above the action bar) */}
        <AnimatePresence mode="wait">
          {activePopover === 'comment' && (
            ...
            >
              <div className="bg-gray-50 rounded-xl p-3">
                <Label className="text-xs text-gray-500 mb-1.5">Comentario</Label>
                <Textarea
                  placeholder="Adicione um comentario..."
                  value={state?.comment || ''}
                  onChange={(e) => onComment(e.target.value)}
                  className="min-h-[56px] bg-white rounded-lg text-sm"
                />
              </div>
            </motion.div>
          )}
          ...
          {activePopover === 'attachment' && (
            ...
            >
              <div className="bg-gray-50 rounded-xl p-3">
                <Label className="text-xs text-gray-500 mb-1.5">Anexos</Label>
                <AttachmentZone
        ...
        {/* Action bar -- centered floating dock */}
        <div className="flex items-center justify-center pt-1">
          <div
            className="bg-gray-50 rounded-full px-3 py-1.5 inline-flex items-center gap-2"
            style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}
          >
            {/* Comment icon button */}
            <button
              onClick={() => handleTogglePopover('comment')}
              className={cn(
                'relative w-7 h-7 rounded-full flex items-center justify-center transition-colors duration-150 cursor-pointer',
              )}
              style={
                activePopover === 'comment'
                  ? { backgroundColor: sectionColor, color: 'white' }
            ...
            >
              ...
              {/* Badge counter */}
              {commentCount > 0 && activePopover !== 'comment' && (
                <span
            ...
            <button
              onClick={() => handleTogglePopover('attachment')}
              className={cn(
                'relative w-7 h-7 rounded-full flex items-center justify-center transition-colors duration-150 cursor-pointer',
              )}
              style={
            ...
            >
              ...
              {/* Badge counter */}
              {attachmentCount > 0 && activePopover !== 'attachment' && (
                <span
                  className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full text-[8px] font-bold text-white flex items-center justify-center"
                  style={{ backgroundColor: sectionColor }}
                >
